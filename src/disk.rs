...
pub fn list_disks() -> Result<Vec<DiskInfo>, String> {
    ...
    let disks = parsed.blockdevices.into_iter()
...
pub fn partition_disk(disk_path: &str) -> Result<(), String> {
    ...
    let cmds = vec![
        ...
        vec!["sgdisk", "-n", "1:0:+512M", "-t", "1:ef00", "-c", "1:boot", disk_path],
        vec!["sgdisk", "-n", "2:0:0", "-t", "2:8300", "-c", "2:root", disk_path],
    ...
    let boot_part = if disk_path.contains("nvme") { format!("{}p1", disk_path) } else { boot_part };
    let root_part = if disk_path.contains("nvme") { format!("{}p2", disk_path) } else { root_part };
    ...
    if !mkfs_boot.status.success() { return Err(String::from_utf8_lossy(&mkfs_boot.stderr).to_string()); }

    let mkfs_root = Command::new("mkfs.btrfs")
        .args(&["-f", &root_part])
        .output()
        .map_err(|e| format!("Failed mkfs.btrfs: {}", e))?;
    if !mkfs_root.status.success() { return Err(String::from_utf8_lossy(&mkfs_root.stderr).to_string()); }

    // Mount and create subvolumes
    let _ = Command::new("umount").args(&["-R", "/mnt"]).output();
    Command::new("mount").args(&[&root_part, "/mnt"]).output().map_err(|e| format!("Mount root fail: {}", e))?;
    
    for subvol in &["@", "@home", "@nix", "@log"] {
        Command::new("btrfs").args(&["subvolume", "create", &format!("/mnt/{}", subvol)]).output().map_err(|e| e.to_string())?;
    }
    Command::new("umount").args(&["-R", "/mnt"]).output().map_err(|e| e.to_string())?;

    // Mount them properly
    Command::new("mount").args(&["-o", "subvol=@,compress=zstd,noatime", &root_part, "/mnt"]).output().map_err(|e| e.to_string())?;
    ...
    Command::new("mount").args(&["-o", "subvol=@home,compress=zstd,noatime", &root_part, "/mnt/home"]).output().map_err(|e| e.to_string())?;
    Command::new("mount").args(&["-o", "subvol=@nix,compress=zstd,noatime", &root_part, "/mnt/nix"]).output().map_err(|e| e.to_string())?;
    Command::new("mount").args(&["-o", "subvol=@log,compress=zstd,noatime", &root_part, "/mnt/var/log"]).output().map_err(|e| e.to_string())?;
    Command::new("mount").args(&[&boot_part, "/mnt/boot"]).output().map_err(|e| e.to_string())?;
