...
pub fn detect_existing_installations() -> Result<Vec<InstallationMatch>, String> {
    ...
    for disk in disks {
        ...
        if let Some(blockdevices) = partitions_json.get("blockdevices")
            && let Some(children) = blockdevices.get(0).and_then(|d| d.get("children"))
            && let Some(parts) = children.as_array()
        {
            for part in parts {
                if let Some(name) = part.get("name").and_then(|n| n.as_str())
                    && let Ok(Some(found)) = check_partition_for_kryonix(name)
                {
                    matches.push(found);
                }
            }
        ...
        && status.success()
    {
        let result = (|| {
            ...
            if flake_lock.exists() || installed_flag.exists() {
                ...
                let version =
                    std::fs::read_to_string(Path::new(&mount_point).join("etc/kryonix-version"))
                        .ok()
                        .and_then(|s| {
                            s.lines()
